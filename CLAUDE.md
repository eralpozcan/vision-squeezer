...
- **All version-bearing manifests must be bumped together** on every release:
  ...
  - `server.json` — the MCP registry manifest (registry.modelcontextprotocol.io). Bump **both** the root `version` and `packages[0].version` (npm package version, must match `package.json`). Published automatically by the **`mcp-publish.yml`** workflow (separate from `release.yml`, on every `v*` tag + `workflow_dispatch`) via `mcp-publisher` using GitHub OIDC — no secret needed. It is kept separate so a registry-only re-publish (e.g. after fixing a validation error) never has to re-run crates.io/npm, which reject an already-published version. **The `description` must be ≤100 chars** (registry validation rejects longer — this bit v0.6.0). Keep the provider list short. (Pre-v0.5.0 this was a manual step and drifted stale — sat at `0.1.1` through 0.3.x.)
